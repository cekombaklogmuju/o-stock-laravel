        ...
        <div class="bg-white rounded-2xl shadow-xl shadow-slate-200 border border-slate-200/80 p-8">
            ...
            <form action="{{ route('register.post') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        <label for="role" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Peran Gudang</label>
                        <div class="relative">
                            <select name="role" id="role" required
                                    class="w-full px-3 py-2.5 text-sm bg-slate-50 border @error('role') border-rose-400 bg-rose-50/30 @else border-slate-300 @enderror rounded-lg focus:bg-white focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 outline-none transition">
                                <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Kepala Gudang (Admin)</option>
                                <option value="cabang" {{ old('role') === 'cabang' || !old('role') ? 'selected' : '' }}>Operator Gudang</option>
                            </select>
                        </div>
                        @error('role')
                            <p class="text-[11px] text-rose-600 mt-1">{{ $message }}</p>
