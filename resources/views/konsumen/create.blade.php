...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-black text-slate-900">Tambah Konsumen Baru</h1>
            <p class="text-xs text-slate-500">Daftarkan pelanggan atau toko pembeli</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-xs">
        <form action="{{ route('konsumen.store') }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama') }}" required placeholder="e.g. Toko Berkah Jaya"
                           class="w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-300 rounded-lg focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="kota" value="{{ old('kota') }}" placeholder="e.g. Jakarta Barat"
                           class="w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-300 rounded-lg focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>
            ...
            <div>
                ...
                <textarea name="alamat" rows="3" placeholder="Alamat pengiriman faktur/barang..."
