...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-black text-slate-900">Edit Salesman</h1>
            <p class="text-xs text-slate-500">Perbarui data kontak dan penempatan cabang</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-xs">
        <form action="{{ route('salesman.update', $salesman->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="id_cabang" required
                        ...
                        @foreach($kantors as $ktr)
                            <option value="{{ $ktr->id }}" {{ old('id_cabang', $salesman->id_cabang) == $ktr->id ? 'selected' : '' }}>
                                {{ $ktr->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $salesman->nama) }}" required
                           class="w-full px-3.5 py-2 text-xs bg-slate-50 border border-slate-300 rounded-lg focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="no_telp" value="{{ old('no_telp', $salesman->no_telp) }}"
