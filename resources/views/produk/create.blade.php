...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah Barang Baru ke Gudang</h1>
            <p class="text-sm text-slate-500">Daftarkan item barang, alokasikan lokasi rak, barcode, dan batas stok minimum</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-xs">
        <form action="{{ route('produk.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama" value="{{ old('nama') }}" required placeholder="Contoh: Motherboard Intel B760 DDR5"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-indigo-50/50 rounded-xl border border-indigo-100">
                <div>
                    ...
                    <input type="text" name="lokasi_rak" value="{{ old('lokasi_rak', 'Rak A-01-A') }}" required
                           placeholder="Contoh: Rak A-01-A / Lorong 2 B"
                ...
                <div>
                    ...
                    <input type="text" name="satuan" value="{{ old('satuan', 'Pcs') }}" required
                           placeholder="Contoh: Pcs, Box, Unit, Tube, Pack, Roll"
                           class="w-full px-3.5 py-2.5 text-sm bg-white border border-indigo-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none font-semibold text-slate-800">
            ...
            <div>
                ...
                <textarea name="spesifikasi" rows="2" placeholder="Dimensi, nomor part manufaktur, voltase, dsb."
                          class="w-full px-3.5 py-2.5 text-sm bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none">{{ old('spesifikasi') }}</textarea>
            </div>
