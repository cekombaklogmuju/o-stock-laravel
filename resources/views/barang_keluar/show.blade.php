...
@section('title', 'Detail Pengeluaran Barang - ' . $barangKeluar->no_keluar)
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 no-print">
        <div>
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Dokumen Barang Keluar</h1>
            </div>
            <p class="text-sm text-slate-500 font-mono mt-0.5">{{ $barangKeluar->no_keluar }}</p>
        </div>
        <div class="flex items-center gap-2">
            <button type="button" onclick="window.print()"
                ...
                <span>Cetak Surat Pengeluaran</span>
            </button>
            <a href="{{ route('barang-keluar.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white font-semibold text-xs rounded-xl shadow-sm transition">
                <i class="fa-solid fa-plus"></i>
                <span>Catat Keluar Baru</span>
            </a>
        </div>
    </div>

    <!-- Printable Dispatch Card -->
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 sm:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-200 pb-6 gap-4">
            <div>
                <div class="flex items-center gap-2.5">
                    ...
                    <span class="text-xl font-black text-slate-900 tracking-wider uppercase">SURAT PENGELUARAN BARANG (DISPATCH)</span>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl text-sm">
            <div>
                <p class="text-xs font-semibold text-slate-400 uppercase">No. Pengeluaran</p>
                <p class="font-bold text-slate-900 font-mono text-base">{{ $barangKeluar->no_keluar }}</p>
            </div>
            <div>
                <p class="text-xs font-semibold text-slate-400 uppercase">Tanggal Keluar</p>
                <p class="font-bold text-slate-900">{{ \Carbon\Carbon::parse($barangKeluar->tanggal)->format('d F Y') }}</p>
            </div>
            <div>
                <p class="text-xs font-semibold text-slate-400 uppercase">Tujuan Pengeluaran</p>
                <p class="font-bold text-slate-900">{{ ucwords(str_replace('_', ' ', $barangKeluar->tujuan)) }}</p>
            </div>
            <div>
                <p class="text-xs font-semibold text-slate-400 uppercase">Penerima / Pemohon</p>
                <p class="font-bold text-slate-900">{{ $barangKeluar->penerima ?: '-' }}</p>
            </div>
        </div>

        @if($barangKeluar->keterangan)
            <div class="p-3 bg-amber-50/50 border border-amber-200/60 rounded-lg text-xs text-amber-900">
                <span class="font-bold">Keterangan / Alasan:</span> {{ $barangKeluar->keterangan }}
            </div>
        @endif
            ...
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-100 text-slate-700 font-bold border-b border-slate-200 text-xs uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-center">Jumlah Keluar</th>
                        <th class="px-4 py-3">Catatan</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @foreach($barangKeluar->items as $index => $item)
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="px-4 py-3.5">
                                <span class="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded bg-amber-50 text-amber-800 border border-amber-200">
                                    <i class="fa-solid fa-table-cells"></i>
                                    {{ $item->produk->lokasi_rak ?: 'Tanpa Rak' }}
                                </span>
                            </td>
                            <td class="px-4 py-3.5 text-center font-bold text-rose-700 text-base">
                ...
                <tfoot class="bg-slate-50 border-t border-slate-200 font-bold text-slate-900">
                    <tr>
                        <td colspan="4" class="px-4 py-3 text-right text-xs uppercase">Total Barang Dikeluarkan:</td>
                        <td class="px-4 py-3 text-center text-rose-700 text-base">
                            {{ $barangKeluar->items->sum('jumlah') }} Unit
                        </td>
                        <td class="px-4 py-3 text-xs text-slate-500">
                            ({{ $barangKeluar->items->count() }} Macam Produk)
                        </td>
                    </tr>
                </tfoot>
            </table>
        ...
        <!-- Signatures for Dispatch Voucher -->
        <div class="pt-8 grid grid-cols-2 sm:grid-cols-3 gap-8 text-center text-xs text-slate-600">
            <div>
                <p class="font-semibold text-slate-800">Petugas Gudang (Pengeluar),</p>
                <div class="h-20"></div>
                <p class="border-t border-slate-300 pt-1 font-semibold text-slate-900">
                    ( {{ $barangKeluar->creator ? $barangKeluar->creator->name : 'Staff Warehouse' }} )
                </p>
            </div>
            <div>
                <p class="font-semibold text-slate-800">Penerima Barang,</p>
                <div class="h-20"></div>
                <p class="border-t border-slate-300 pt-1 font-medium">
                    ( {{ $barangKeluar->penerima ?: '........................................' }} )
                </p>
            </div>
            <div class="hidden sm:block">
                <p class="font-semibold text-slate-800">Kepala Gudang (Menyetujui),</p>
                <div class="h-20"></div>
